...
1 초	256 MB	12842	3284	2463	23.845%
문제
...
while queue:
    ...
    if load[y][x] == 2:
                ...
                gram[1] = y
                gram[2] = visit[y][x]
                gram[3] = gram[2]+N-y-1+M-x-1
    for i in range(4):
        nx = x + dx[i]
        ny = y + dy[i]
        if 0 <= nx < M and 0 <= ny < N and visit[ny][nx] == -1 and (load[ny][nx] == 0 or load[ny][nx] == 2):
...
if visit[N-1][M-1] == -1:
    ...
    low_dist = visit[N-1][M-1]
else:
    if visit[N-1][M-1] <= gram[3]:
         low_dist = visit[N-1][M-1]
    else:
         low_dist = gram[3]


if low_dist == -1 or low_dist > T:
    print('Fail')
else:
    print(low_dist)
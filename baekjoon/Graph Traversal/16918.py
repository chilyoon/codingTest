...
bomb = []
for _ in range(R):
    bomb.append(list(map(str, input())))
visit = [['O']*C for _ in range(R)]
queue = deque()
visit2 = [['O']*C for _ in range(R)]
queue2 = deque()
...
while queue:
    ...
    for i in range(4):
        nx = x + dx[i]
        ny = y + dy[i]
        if 0 <= nx < R and 0 <= ny < C and visit[nx][ny] == 'O':
            if bomb[nx][ny] == '.':
                visit[nx][ny] = '.'
...
while queue2:
    ...
    for i in range(4):
        nx = x + dx[i]
        ny = y + dy[i]
        if 0 <= nx < R and 0 <= ny < C and visit2[nx][ny] == 'O':
            if visit[nx][ny] == '.':
                visit2[nx][ny] = '.'
